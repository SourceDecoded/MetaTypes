    ...
    _createClass(EntityBuilder, [{
        ...
        value: function _attachEntityRelationships(tableName, entity, entityMap, attachedEntities) {
            ...
            oneToOneRelationships.forEach(function (relationship) {
                ...
                var target = Object.values(entityMap[foreignTableName]).find(function (target) {
            ...
            oneToManyRelationships.forEach(function (relationship) {
                ...
                var targets = Object.values(entityMap[foreignTableName]).filter(function (target) {
                    return target[foreignKey] === entity[key];
                });
            ...
            targetRelationships.forEach(function (relationship) {
                ...
                var source = Object.values(entityMap[sourceTableName]).find(function (source) {
        ...
        value: function _convertRow(row, entityMap) {
            ...
            var edm = this.edm;
            var name = this.name;
            var key = this._getKeyForEntity(entity);
            ...
            Object.keys(row).forEach(function (key) {
                ...
                var entity = _this._createEntity(tableName, row);
        ...
        value: function _createEntity(type, row) {
            ...
            columns.forEach(function (column) {
                entity[column.name] = _this2._convertValue(row["" + type + delimiter + column.name]);
            });
        ...
        value: function _getKeyForEntity(entity) {
            return this._getPrimaryKeys(type).map(function (key) {
                return entity[key];
            }).join("|");
        }
    }, {
        key: "_getPrimaryKeys",
        value: function _getPrimaryKeys(type) {
            return this._getTable(type).columns.filter(function (column) {
        ...
        value: function _getTable(name) {
            return this.edm.table.columns.find(function (table) {
        ...
        value: function convert(sqlResults) {
            ...
            if (sqlResults.length > 0) {
                ...
                var results = sqlResults.map(function (row) {
                    return _this3.convertRow(row, entityMap);
                });
                ...
                Object.keys(entityMap).forEach(function (key) {
                    ...
                    _this3._attachEntityRelationships(tableName, entity, entityMap, []);
                });
