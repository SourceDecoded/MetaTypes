...
var TableStatementBuilder = function () {
    ...
    _createClass(TableStatementBuilder, [{
        ...
        value: function _wrapIfTableExists(table, schema, query) {
        ...
        value: function createDropTableStatement(schema, table) {
        ...
        value: function createForeignKeysStatement(schema, table, relationships) {
            ...
            return tableRelationships.map(function (relationship) {
                return _this6.createForeignKeyStatement(schema, relationship);
            }).join("/n/t");
        }
    }, {
        ...
        value: function createTableStatement(schema, table, relationships) {
            ...
            if (columnDefinitionsStatement && foreignKeysStatement) {
                return this._wrapIfTableExists(table, schema, "CREATE TABLE [" + schema + "].[" + table.name + "] (" + columnDefinitionsStatement + ", " + foreignKeysStatement + ")");
            } else if (columnDefinitionsStatement) {
                return this._wrapIfTableExists(table, schema, "CREATE TABLE [" + schema + "].[" + table.name + "] (" + columnDefinitionsStatement + ")");
            } else {
                return this._wrapIfTableExists(table, schema, "CREATE TABLE [" + schema + "].[" + table.name + "]");
            }
        }
    }, {
