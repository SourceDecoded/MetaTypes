    ...
    _createClass(_class, [{
        ...
        value: function _checkEdmDbExistsAsync(pool) {
            ...
            return new Promise(function (resolve, reject) {
                pool.query("SELECT * FROM INFORMATION_SCHEMA.TABLES \n            WHERE TABLE_SCHEMA = " + _this3.options.edmSchema + " \n            AND  TABLE_NAME = " + _this3.options.edmTable).then(function (result) {
                    if (result.length === 1) {
                        resolve(true);
                    } else {
                        resolve(false);
        ...
        value: function _verifyEdmTableAsync() {
            ...
            return this.getEdmDbAsync().then(function (pool) {
                _this4._checkEdmDbExistsAsync(pool).then(function (exists) {
                    if (exists) {
                        return pool;
                    } else {
                        return pool.query(generateEdmCreateSql(_this4.options)).then(function () {
                            return pool;
                        });
                    }
                });
