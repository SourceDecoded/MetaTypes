    ...
    _createClass(_class, [{
        ...
        value: function getEdmAsync(name, version) {
            ...
            return this.getEdmDbAsync().then(function (pool) {
                return pool.request().query("SELECT * FROM [" + _this4.options.edmSchema + "].[" + _this4.options.edmTable + "] " + ("WHERE [version]='" + version + "' AND [name]='" + name + "'")).then(function (result) {
            ...
            return this.getEdmAsync(name, version).then(function (edm) {
                if (edm) {
                    ...
                    return _this5.getEdmDbAsync().then(function (pool) {
                        return pool.request().query("INSERT INTO [" + _this5.options.edmSchema + "].[" + _this5.options.edmTable + "] " + ("(name, version, json) VALUES ('" + name + "', '" + version + "', '" + JSON.stringify(newEdm) + "')"));
                    });
                }
            });
        }
        ...
        value: function deleteEdmAsync(name, version) {
            ...
            return this.getEdmDbAsync().then(function (pool) {
                var query = "DELETE FROM [" + _this6.options.edmSchema + "].[" + _this6.options.edmTable + "] " + ("WHERE [version]='" + version + "' AND [name]='" + name + "'");
        ...
        key: "getMigrator",
        value: function getMigrator(iDb) {
            return new _Migrator2.default(iDb);
        }
    }, {
        key: "dispose",
        ...
        value: function _checkEdmDbExistsAsync(pool) {
            ...
            return new Promise(function (resolve, reject) {
                var q = "SELECT * FROM INFORMATION_SCHEMA.TABLES " + ("WHERE TABLE_SCHEMA = '" + _this8.options.edmSchema + "' ") + ("AND TABLE_NAME = '" + _this8.options.edmTable + "'");
