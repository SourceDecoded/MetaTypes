...
exports["migration.Runner.migrateAction: addTableAsync, addColumnAsync with consequential commands and with a invalid action and successfully rollback."] = function () {
    var addTableAsyncCount = 0;
    var addColumnAsyncCount = 0;
    var updateColumnAsyncCount = 0;
    ...
    var migrator = {
        ...
        addColumnAsync: function addColumnAsync(options) {
            addColumnAsyncCount++;

            var newColumn = Object.assign({}, options.column, {
                isIndexed: true
            });
            ...
            return [commandBuilder.createUpdateColumnCommand(options.tableName, options.column, newColumn), invalidCommand];
        },
        removeTableAsync: function removeTableAsync() {
            addTableAsyncCount--;
        },
        ...
        updateColumnAsync: function updateColumnAsync(options) {
            if (updateColumnAsyncCount === 0) {
                _assert2.default.equal(options.column.isIndexed, true);
            } else if (updateColumnAsyncCount === 1) {
                _assert2.default.equal(options.column.isIndexed, false);
            }
    ...
    return runner.migrateAsync([tableCommand, columnCommand]).then(function () {
        ...
        _assert2.default.equal(addTableAsyncCount, 0);
        _assert2.default.equal(addColumnAsyncCount, 0);
        _assert2.default.equal(updateColumnAsyncCount % 2, 0);
    });
};
