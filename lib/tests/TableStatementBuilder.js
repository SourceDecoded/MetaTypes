...
exports["TableStatementBuilder.createTableStatement: Without Relationships."] = function () {
    ...
    assert.equal(tableStatement, "CREATE TABLE 'Source' ('id' NUMERIC PRIMARY KEY AUTOINCREMENT, 'string' TEXT, 'number' NUMERIC, 'date' NUMERIC, 'boolean' NUMERIC, 'float' REAL)");
};
...
exports["TableStatementBuilder.createTableStatement: With Relationships."] = function () {
    ...
    assert.equal(tableStatement, "CREATE TABLE 'Foreign' ('id' NUMERIC PRIMARY KEY AUTOINCREMENT, 'foreignKey' INTEGER) FOREIGN KEY ('foreignKey') REFERENCES 'Source' ('id')");
};
